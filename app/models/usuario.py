...
def buscar_por_id(user_id: int):
    sb = get_supabase()
    res = sb.table(TABLE).select("id, nome, email, id_cargo, nivel_acesso, id_setor, data_criacao").eq("id", user_id).single().execute()
    return res.data
...
def atualizar_por_id(user_id: int, dados: dict):
    sb = get_supabase()
    res = (
        sb.table(TABLE)
        .update(dados)
